  ...
  useEffect(() => {
    ...
    const fetchActivityDetail = async () => {
      try {
        ...
        setRecommendationText(response.data.recommendationText);
      } catch (error) {
        console.error(error);
      }
    }
            ...
            {recommendationText && (
                <Card>
                    <CardContent>
                        ...
                        <Typography paragraph>{activity.recommendationText}</Typography>
                        
                        <Divider sx={{ my: 2 }} />
                        
                        <Typography variant="h6">Improvements</Typography>
                        {activity?.improvementTips?.map((improvement, index) => (
